...
local function send(sock, opcode, message)
    ...
    local chunk_length = 4000
    local start_pos = 1

    local data = {}

    while start_pos <= length do
        local end_pos = math.min(start_pos + chunk_length - 1, length)
        local chunk = message:sub(start_pos, end_pos)
        local msgbyte = { chunk:byte(1, #chunk) }
        for i = 1, #chunk do
            msgbyte[i] = bxor(msgbyte[i], mask_key[(i - 1) % 4 + 1])
        end
        data[#data + 1] = string.char(unpack(msgbyte))
        start_pos = end_pos + 1
    end

    local all = table.concat(data)
    sock:send(all)
end
