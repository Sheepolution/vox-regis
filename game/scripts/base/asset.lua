...
function Asset.font(path, size, force, hinting)
	...
	if force or not fontCache[path] or not fontCache[path][size] then
		for i, v in ipairs(font_formats) do
			local fullPath = "assets/fonts/" .. path .. v
			local info     = love.filesystem.getInfo(fullPath)
			if info then
				font = love.graphics.newFont("assets/fonts/" .. path .. v, size, hinting)
		...
		assert(font, "No font found named " .. path, 2)
	else
