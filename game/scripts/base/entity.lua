...
local AngularMovement = require "base.angular_movement"
...
function Entity:rotateTowardsAngle(angle, amount)
	self.angle = math.rotate(self.angle, angle, amount)
end

function Entity:rotateTowardsEntity(entity, amount)
	self.angle = math.rotate(self.angle, self:getAngle(entity), amount)
end
...
function Entity:addHitbox(x, y, width, height, private, centered)
	local hb = Hitbox(self, x, y, width, height, private, centered)
...
function Entity:overlappable(e)
	...
	local myCache = self.ignoreOverlapCache and self.ignoreOverlapCache[e.tag] or nil
	if myCache ~= nil then
		return not myCache
	end

	local theirCache = e.ignoreOverlapCache and e.ignoreOverlapCache[self.tag] or nil
	if theirCache ~= nil then
		return not theirCache
	end

	self.ignoreOverlapCache[e.tag] = self.ignoreOverlap and self:hasListClass(self.ignoreOverlap, e) or false
	e.ignoreOverlapCache[self.tag] = e.ignoreOverlap and e:hasListClass(e.ignoreOverlap, self) or false

	return not (self.ignoreOverlapCache[e.tag] or e.ignoreOverlapCache[self.tag])
end
...
function Entity:collectCollisionInfo(e, myHitbox, theirHitbox)
	...
	ci.theirTop = myHitbox.lbb.bottom <= theirHitbox.lbb.top
	if ci.theirTop then
		ci.theirBottom = false
	else
		ci.theirBottom = myHitbox.lbb.top >= theirHitbox.lbb.bottom
	end

	ci.theirLeft = myHitbox.lbb.right <= theirHitbox.lbb.left
	if ci.theirLeft then
		ci.theirRight = false
	else
		ci.theirRight = myHitbox.lbb.left >= theirHitbox.lbb.right
	end

	ci.myBottom = ci.theirTop
	ci.myTop = ci.theirBottom
	ci.myLeft = ci.theirRight
	ci.myRight = ci.theirLeft

	local cb = e._collisionInfo
	cb.e = self
	cb.myHitbox = theirHitbox
	cb.theirHitbox = myHitbox
	cb.theirTop = ci.myTop
	cb.theirBottom = ci.myBottom
	cb.theirLeft = ci.myLeft
	cb.theirRight = ci.myRight
	cb.myTop = ci.theirTop
	cb.myBottom = ci.theirBottom
	cb.myLeft = ci.theirLeft
	cb.myRight = ci.theirRight
end

function Entity:handleOverlap(e, myHitbox, theirHitbox, ignoreCorners)
	if e == self then return end
	if self.tile and e.tile then return end
...
function Entity:handleSeparation(collisionInfo)
	...
	local axis = (collisionInfo.theirRight or collisionInfo.theirLeft) and "x" or "y"
...
function Entity:onSeparate(collisionInfo)
	local e = collisionInfo.e
	local axis = (collisionInfo.theirRight or collisionInfo.theirLeft) and "x" or "y"
...
function Entity:addIgnoreOverlap(...)
	...
	if not t.ignoreOverlap then
		t.ignoreOverlap = {}
		t.ignoreOverlapCache = {}
	end

	for i, v in ipairs({ ... }) do
		table.insert(t.ignoreOverlap, v)
	end
end

function Entity:addIgnoreCollision(...)
	local t = getmetatable(self)
	if not t.ignoreCollision then
		t.ignoreCollision = {}
		t.ignoreCollisionCache = {}
	end

	for i, v in ipairs({ ... }) do
		table.insert(t.ignoreCollision, v)
	end
end

function Entity:addImmovableObjects(...)
	local t = getmetatable(self)
	if not t.immovableObjects then
		t.immovableObjects = {}
	end

	for i, v in ipairs({ ... }) do
		table.insert(t.immovableObjects, v)
	end
end
...
function Entity:playSFX(path)
	if self.sfx then
		self.sfx:play(path)
	else
		self._global_sfx:play(path)
	end
end
