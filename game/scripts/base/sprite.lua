...
function Sprite:new(x, y, img, width, height, margin, byframes)
	...
	self.pixelPerfect = CONFIG.defaultPixelPerfect
...
function Sprite:update(dt)
	...
	if self.shader and self._shaders[self.shader].names then
	...
	if #self._masks > 0 then
		self._maskCanvas:renderTo(function()
			love.graphics.clear()
		...
		self:send(self.maskGradient and "mask_gradient_image" or "mask_image", self._maskCanvas)
	end
...
function Sprite:draw()
	...
	local pos = self.pixelPerfect and math_round or _.identity
	...
	if hasBorder or self.singleColor then
		...
		love.graphics.translate(
			pos(self.x + self.origin.x + self.offset.x - self._imageMargin),
			pos(self.y + self.origin.y + self.offset.y - self._imageMargin))
		...
		if self.anim.hasAnimation then
			...
			if hasBorder then
				...
				love.graphics.draw(self.image,
					...
					pos(self.origin.x + self._imageMargin), pos(self.origin.y + self._imageMargin),
					self.shear.x, self.shear.y)
			end
		else
			if hasBorder then
				...
				love.graphics.draw(self.image,
					pos(self.x + self.origin.x + offsetX - self._imageMargin),
					pos(self.y + self.origin.y + offsetY - self._imageMargin),
					self.angle + self.angleOffset,
					self.scale.x * (self.flip.x and -1 or 1), self.scale.y * (self.flip.y and -1 or 1),
					pos(self.origin.x + self._imageMargin), pos(self.origin.y + self._imageMargin),
					self.shear.x, self.shear.y)
...
function Sprite:getDrawCoordinates(center)
	local pos = self.pixelPerfect and math_round or _.identity
	local offsetX, offsetY = self:getOffset()
	local ox, oy = pos(self.origin.x + self._imageMargin), pos(self.origin.y + self._imageMargin)
...
function Sprite:setImage(path, width, height, margin, byframes)
	...
	if not width and type(path) == "string" then
		if Sprite._jsonCache[path] == nil then
			...
			if json_info then
				...
				Sprite._jsonCache[path] = data
			else
				Sprite._jsonCache[path] = false
			end
		end

		local data = Sprite._jsonCache[path]
		if data then
			...
			self._imageMargin = math.floor(margin / 2)
	...
	if width or Sprite._jsonCache[path] == false then
		margin = margin or (width and 1 or 0)
		self._imageMargin = margin
		...
		if byframes ~= false then
			...
			if width ~= math_round(width) then
...
function Sprite:addBorder(color, eight, size)
	if DEBUG then
	...
	if Sprite._borderCache[self.image] then
		self.image = Sprite._borderCache[self.image]
		return
	end
	...
	Sprite._borderCache[old_img] = self.image
end
...
function Sprite:addShader(name, ...)
	...
	if not ... then
		...
		if type(list[1]) == "userdata" then
			self._shaders[name] = { shader = list[1], direct = true }
			self.shader = name
			return list[1]
		end
	end
	...
	self.shader = name
	return self._shaders[name].shader
end
...
function Sprite:send(extern, ...)
	...
	if not extern:find("_") and not self._shaders[self.shader].direct then
...
function Sprite:drawAsChild(p, topleft, props, kind)
	...
	local all_props = { "offset", "scale", "flip", "visible", "color", "alpha", "angle", "angleOffset" }
...
function Sprite:addMask(mask, gradient)
	...
	if #self._masks == 1 then
		...
		self.maskGradient = gradient
	end

	return mask
end

function Sprite:removeMask(mask)
	for i, v in ipairs(self._masks) do
		if v == mask then
			table.remove(self._masks, i)
			break
		end
	end

	if #self._masks == 0 then
		self._maskCanvas = nil
		self:removeShader(self.maskGradient and "mask_gradient" or "mask")
	end
end
