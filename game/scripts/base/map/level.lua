...
local TileLayer = require "base.map.tile_layer"
local MapUtils = require "base.map.map_utils"
...
function Level:new(scene, world, levelData, layerDefinitions, entityDefinitions, properties, mapProperties)
    ...
    if levelData.identifier:find("Level_") then
    ...
    if mapProperties.layer then
        for i, v in ipairs(mapProperties.layer) do
            table.insert(self.decorationLayers, v.decoration and true or false)
        end
    end
