...
function mt_anim:onAnyFrame(func)
	if func then
		local f = {}
		f.func = func
		self._oFrames[-1] = f
...
function Animation:update(dt)
	...
	if self._timer == 1 then
		...
		local on_any_frame_function = current._oFrames[-1]
		if on_any_frame_function then
			on_any_frame_function.func(self.frame, self.loops)
		end
	end
	...
	if self._timer > #current.frames + 1 or self._timer < 1 then
		...
		if current._after then
			if current._afterLoop == self.loops + 1 then
				-- Call the onComplete function before changing the animation
				if current._ocFunc then
					if not current._ocFuncFirst or self.loops == 0 then
						current._ocFunc(self.loops)
						if current ~= self._current then return end
	...
	if self.frame ~= oldframe then
		...
		local on_frame_function = current._oFrames[self.frame]
		if on_frame_function then
			if not on_frame_function.first or self.loops == 1 then
				on_frame_function.func()
			end
		end
	end
...
function Animation:set(name, force)
	...
	if self.name ~= name or force then
		...
		local on_any_frame_function = self._current._oFrames[-1]
		if on_any_frame_function then
			on_any_frame_function.func(self.frame, self.loops)
		end

		local on_frame_function = self._current._oFrames[self.frame]
		if on_frame_function then
			on_frame_function.func()
		end
	end
...
function Animation:setFrame(frame)
	self.frame = _.clamp(frame, 1, #self._current.frames)

	if self.frame ~= frame then
		warning("The frame " .. frame .. " doesn't exist in the animation " .. self.name .. ".")
	end
end
...
function Animation:getQuadFrame()
	return self._current.frames[self.frame]
end
